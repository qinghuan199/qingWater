<template>
	<view class="content">
		...
		<customer></customer>
		<view class="m-t-50">
			<view class="font-size-13 p-20" style="color: #000;">
				Dear Agent, to keep your funds secure, please do not enter your bank details. Our agents will never ask
				for this information. Thank you for your cooperation
			</view>
		</view>
		<view class="u-flex-y p-30 width-690 radius-12 u-flex-self-center m-t-30"
			style="background-color: #fff;margin: 0 auto;box-shadow: 0px 2px 4px 2px rgba(226, 246, 224, .5);">
			...
			<view class="u-flex-x u-flex-between m-t-30">
				<text class="label">{{$t('bind.name')}}</text>
				<input class="uni-input text-right  " type="text" :disabled="disabled"
					:placeholder="$t('bind.name_desc')" v-model="bank_name" />
			</view>
			<u-line class="m-t-30"></u-line>
			<view class="u-flex-x u-flex-between m-t-30">
				<text class="label" style=" flex-shrink:0;">{{$t('bind.usdt_address')}}</text>
				<input class="uni-input text-right  " type="text" :disabled="disabled"
			...
			<view class="u-flex-x u-flex-between m-t-30">
				...
				<input class="uni-input text-right  " type="text" :disabled="disabled"
					:placeholder="$t('bind.phone_desc')" v-model="phone" />
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				var that = this;
				var userInfo = getApp().getCache('userInfo');
				if (getApp().isEmpty(that.name) || getApp().isEmpty(that.usdt_address) || getApp().isEmpty(that.phone) ||
					getApp().isEmpty(
						that.bank_name)) {
					uni.showToast({
