				...
				<view class="popup-bg">
					<view class="popup-content" :class="{'popup-content-show' : popup_show}">
						<view class="update-wrap">
							...
							<view class="content">
								<text class="title">{{$t('update.find')}} V{{update_info.version}}</text>
								<!-- 升级描述 -->
								<view class="title-sub" v-html="update_info.note"></view>
								<!-- 升级按钮 -->
								<button class="btn" v-if="downstatus < 1" @click="onUpdate()">{{$t('update.update')}}</button>
								<!-- 下载进度 -->
								<view class="sche-wrap" v-else>
									...
									<text class="down-text">{{$t('update.progress')}}:{{(downSize/1024/1024 ).toFixed(2)}}M/{{(fileSize/1024/1024).toFixed(2)}}M</text>
								</view>
							</view>
						</view>
	...
	export default {
		...
		onLoad(option) {
			...
			if (option.updata_info) {
				...
				plus.nativeUI.toast(that.$t('update.toast1'));
				setTimeout(() => {
					uni.navigateBack()
				}, 500)
			}
		...
		methods: {
					...
					uni.getNetworkType({
						success: (res) => {
							if (res.networkType == "wifi") {
								...
								uni.showModal({
									title: that.$t('update.toast2'),
									content: that.$t('update.toast3'),
									success: (modal_res) => {
			...
			startUpdate() {
				...
				if (/\.apk$/.test(this.update_info.now_url)) {
					...
					plus.runtime.openURL(this.update_info.now_url, function() { //调用外部浏览器打开更新地址
						plus.nativeUI.toast(that.$t('update.toast4'));
					});
				}
			},
			// 下载升级资源包
			download_wgt() {
				var that = this;
				plus.nativeUI.showWaiting(that.$t('update.toast5')); //下载更新文件...
				let options = {
					method: "get"
				};
				...
				dtask.addEventListener("statechanged", (task, status) => {
					if (status === null) {} else if (status == 200) {
						...
						plus.nativeUI.closeWaiting();
						plus.nativeUI.toast(that.$t('update.toast6'));
						this.downing = false;
						this.downstatus = 0;
					}
				});
			...
			installWgt(path) {
				var that = this;
				plus.nativeUI.showWaiting(that.$t('update.toast7')); //安装更新文件...
				plus.runtime.install(path, {}, function() {
					plus.nativeUI.closeWaiting();
					// 应用资源下载完成！
					plus.nativeUI.alert(that.$t('update.toast8'), function() {
						plus.runtime.restart(); //重启APP
					});
				}, function(e) {
					plus.nativeUI.closeWaiting();
					// 安装更新文件失败
					plus.nativeUI.alert(that.$t('update.toast9')+"[" + e.code + "]：" + e.message);
				});
			},
			// 取消更新
			closeUpdate() {
