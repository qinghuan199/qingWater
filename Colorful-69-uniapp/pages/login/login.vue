	...
	<view class="content u-flex-y u-flex-items-center">
		...
		<view class="login-form u-flex-y   ">
			...
			<view class="u-flex-x u-flex-items-center p-20-30 m-t-30 login-input ">
				...
				<input class=" p-l-30" placeholder-style="color:#666;" maxlength="11" type="text"
					:placeholder="$t('login.account_desc')" v-model="phone" />
			</view>
			<view class="u-flex-x p-20-30 u-flex-items-center  m-t-30 login-input m-b-100">
				<view class="">
					<!-- <view class="font-error u-label">{{$t('login.password')}}</view> -->
					<!-- <image src="../../static/gof/g-9-02.png" mode="" class="icon-45 m-t-10"></image> -->
				</view>
				<!-- <input class="m-l-20" maxlength="15" type="password" :placeholder="$t('login.password_desc')"
					v-model="password" placeholder-style="color:#000" /> -->
			...
			<view class="u-flex u-flex-center">
				<view @click="onLogin" class="login-btn u-flex-x  u-flex-center u-flex-items-center m-b-50">
					登录
				</view>
			</view>
			<view class="login-btn1  u-flex-x  u-flex-center u-flex-items-center  text-center font-size-13  "
				@click="openWin('register')" style="">
				注册
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			onLogin() {
				...
				getApp().getData('api/login', postData, function(res) {
