<template>
	<view class="game-recharge-page">
		...
		<view class="header">
			...
			<text class="header-title">游戏转账</text>
			...
			<view class="quick-amounts">
				...
				<view class="quick-buttons">
					<view 
						...
						:key="amount" 
						:class="['quick-btn', { 'selected': transferAmount == amount }]"
		...
		<view class="transfer-btn-container">
			...
			>
				<text class="transfer-btn-text">转账</text>
			</button>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			getUserInfo() {
				...
				getApp().getData('api/getuserinfo', postData, function(res) {
					if (res.code === 200) {
						...
						// 更新缓存
						getApp().setCache('userInfo', res.data);
					}
				}, function(error) {
			...
			// 设置最大金额
			setMaxAmount() {
				const maxAmount = parseFloat(this.userInfo.money || 0);
				this.transferAmount = maxAmount.toFixed(2);
			},

			// 选择快捷金额
			selectQuickAmount(amount) {
				this.transferAmount = amount.toString();
			},
			...
			handleTransfer() {
				...
				const balance = parseFloat(this.userInfo.money || 0);
			...
			processPayment(orderId) {
				...
				getApp().getData('api/order/pay', postData, function(res) {
					if (res.code === 200) {
						uni.showModal({
							title: '转账成功',
							content: `已成功转账¥${that.transferAmount}到${that.gameName}`,
							showCancel: false,
							success: function() {
								// 转账成功后返回上一页
								uni.navigateBack();
...
<style lang="scss" scoped>
	...
	.quick-btn.selected .quick-btn-text {
		color: #fff;
	}
